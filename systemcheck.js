...
const JOBS = [
  ...
  {
    id: "ap_pace", label: "إيقاع اليوم (كل ٥ دقايق)", everyMin: 5, graceMin: 40,
    what: "بتقيس مبيعات النهاردة لحد دلوقتي وبتزوّد أو تقلّل الميزانية جوّه اليوم.",
    selfHeals: "أيوه — دورة مستقلة كل ٥ دقايق.",
  },
  {
    id: "ap_daypart", label: "نافذة الإعلانات (فتح/قفل)", everyMin: 5, graceMin: 40,
...
export function register(app, ctx, deps = {}) {
  ...
  async function build(auth) {
    ...
    const jobs = JOBS.map((j) => {
      ...
      // الناشر والدايبارت بيسكتوا بحق لما مفيش شغل — السكوت مش عطل.
      if (state === "late" && j.id === "content_pub" && !live.nextDue) {
        state = "idle"; line = j.quietOk;
      }
      if (state === "late" && j.id === "ap_daypart") {
        state = "idle";
        line = `مفيش وقف/تشغيل من ${age} دقيقة — طبيعي لو النافذة فضلت مفتوحة. `
          + `التأكيد الحقيقي إن الإعلانات شغالة بيجي من فحص النافذة تحت.`;
      }
