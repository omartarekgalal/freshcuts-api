/* ═══════════════════════════════════════════════════════════════════════════
   ...
   ── تلات أنواع من العروض، تلات بصمات مختلفة ──────────────────────────────
   بعد ما عمر قال إن **العرضين** آخرهم ٣١ أغسطس، وبعد ما اتزاد عرض اليوم
   الوطني، بقى في الملف ده تلات عروض — وكل واحد فيهم بيتقاس بطريقة تانية:
     ...
     • nd96 (هدية اليوم الوطني) — **مش تركيبة ولا صنف**: ده **شرط على
       الفاتورة**. فاتورتك وصلت ٩٦ ريال؟ خد بيتزا هدية. بصمته إجمالي الطلب
       نفسه (`detect.mode = "threshold"`) — أدق بصمة في الملف ده، لأنها
       بتتقرا من `ts_orders.total` ومش محتاجة الكاشير يعمل أي حاجة صح عشان
       القياس يشتغل. شوف تعليق العرض تحت.
   ...
   nd96 أول عرض في الملف ده ليه `from` في المستقبل. `offerState()` بترجّع
   `started:false` قبل اليوم ده، و`activeOffers()` مش بتشوفه — يعني هو
   **مسجّل ومش معلن**. وده مقصود: العرضين القدام آخرهم ٣١ أغسطس، وعرض
   اليوم الوطني بيبتدي ١٧ سبتمبر، فمفيش يوم واحد بيتلاقوا فيه على الشاشة.
   `publicOffer()` بترجّع `upcoming` و`startsIn` عشان الحُرّاس يفرّقوا بين
   «العرض ده انتهى» و«العرض ده لسه مابدأش» — الحالتين الاتنين بيمنعوا النشر،
   بس السبب مختلف والرسالة لازم تقول الصح.
...
export const OFFERS = [
  ...
  /* ── هدية اليوم الوطني ٩٦ ────────────────────────────────────────────────
     العرض ده **مش تركيبة ولا صنف** — ده أول عرض في المطعم آليّته شرط على
     الفاتورة: وصلت ٩٦ ريال؟ خد بيتزا تشيكن رانش هدية.

     ليه الشكل ده بالذات، وده السؤال اللي عمر طلبه بالنص («لازم نحدد أهدافنا
     من العروض إيه لو هنتنازل في الأسعار»):

       ١) **الهدف هو حجم الفاتورة، مش سعر الصنف.** الأيام اللي كانت مبيعاتها
          أعلى ماكانتش أيام زحمة — كانت أيام سلة أكبر (٣٫٨–٤٫٠ صنف في الطلب
          مقابل ٢٫٤–٢٫٩ عادي) بنفس سعر الصنف تقريبًا. يعني الرافعة المثبتة
          هنا هي **عدد الأصناف في الطلب**. شرط على الفاتورة بيضرب على الرافعة
          دي مباشرة؛ خصم على سعر صنف بيضرب على الرافعة الغلط.

       ٢) **مفيش تنازل في السعر أصلاً.** الفاتورة بتتدفع كاملة، والتنازل
          الوحيد هو تكلفة البيتزا الهدية — رقم معروف ومسقوف ومابيتدفعش غير
          لما الفاتورة تكون عدّت ٩٦ فعلاً. قارن بخصم ٢٠٪ على المنيو: بهامش
          مساهمة ٤٥٫٢٪ للصالة، خصم ٢٠٪ بيحتاج **زيادة ٧٩٪ في عدد الطلبات**
          عشان يقف مكانه بس. مفيش سبب نتوقع رقم زي ده.

       ٣) **البيتزا هي أنيم فئة عندنا** (٣٫٥٪ من الإيراد من ١ مايو لـ٢٥
          أغسطس، مقابل ٢٤٫٨٪ للكريب و٢٤٫٦٪ للوجبات) وهي مش بتزاحم الفحم.
          الهدية بتجرّب الفئة دي على عميل موجود بالفعل جوّه المطعم.

       ٤) **مفيش لبس في نقطة البيع.** العرضين القدام «داخل الصالة فقط»،
          ومع ذلك ١٢ من ٤٢ صينية و٦ من ١٤ كومبو اتدقّوا Take away. العرض ده
          شغّال في **الصالة والتيك أواي بنفس الشكل**، فمفيش خانة يغلط فيها
          الكاشير أصلاً — الطريقة الوحيدة لإلغاء خطأ هي إلغاء الاختيار.

     غير مشمول: تطبيقات التوصيل. هامش المساهمة هناك ٢٤٫٥٪ بعد عمولة ٤٦٫٨٪،
     فبيتزا هدية على فاتورة تطبيق بتاكل الهامش كله. ولاحظ الرقم ده: فاتورة
     ٩٦ ريال في الصالة **مع** البيتزا الهدية بتسيب ٢٨٫٠ ر.س مساهمة، وهي
     أكتر من الـ٢٣٫٥ اللي بتسيبها نفس الفاتورة على كيتا من غير أي هدية.

     ⚠️ العرض ده محتاج خطوة تشغيلية واحدة قبل ١٧ سبتمبر: صنف في نقطة البيع
     اسمه «هدية اليوم الوطني» بسعر صفر. من غيره الكاشير هيضطر يخصم بإيده على
     سطر البيتزا — وده بالظبط سطح الخطأ اللي عرض الـ٧٠ اتعلّمنا منه. */
  {
    id: "nd96",
    productId: "offer-nd96",
    /* الاسم ده بيتكتب بالحرف في التصميم كمان — price-guard بيطابق بالاسم. */
    catalogTitle: "بيتزا هدية مع كل فاتورة ٩٦ ريال",
    title: "هدية اليوم الوطني ٩٦",
    desc: "فاتورتك ٩٦ ريال أو أكثر؟ بيتزا تشيكن رانش هدية. داخل الفرع — صالة"
      + " أو تيك أواي، وغير سارٍ على تطبيقات التوصيل",
    emoji: "🎁",
    image: "",
    /* ٩٦ هنا **مش سعر العرض** — دي أقل قيمة فاتورة تستحق الهدية. الحقل اسمه
       `price` عشان الحُرّاس وسجلّ العروض بيقروا الحقل ده بالاسم ده، لكن
       `priceRole` بيقول لأي سطح بيقرا إن الرقم ده حد أدنى مش تسعيرة. أي
       واجهة بتكتب «٩٦ ريال» جنب العنوان لازم تكتب معاها «فأكثر». */
    price: 96,
    priceRole: "min_bill",
    currency: "SAR",
    mechanic: "threshold",
    /* أول عرض مش «داخل الصالة فقط». النطاق مكتوب صريح هنا بدل ما كل سطح
       يستنتجه من dineInOnly. */
    dineInOnly: false,
    note: "داخل الفرع — صالة أو تيك أواي · غير سارٍ على تطبيقات التوصيل",
    /* الهدية: صنف حقيقي من المنيو الحي بسعره الحقيقي. الرقم ده هو اللي
       بيتحسب عليه التنازل — مش تخمين. */
    gift: { name: "بيتزا تشيكن رانش", productId: "43", menuPrice: 28 },
    /* اسم السطر بصفر ريال في نقطة البيع. ده **إشارة الالتزام** بس، مش أساس
       القياس: القياس بيتقرا من إجمالي الفاتورة (تحت)، فلو الكاشير نسي السطر
       الرقم بيفضل صادق ونعرف إنه نسي. */
    giftLine: "هدية اليوم الوطني",
    /* الهدف مش اكتساب زي العرضين التانيين — الهدف حجم السلة. الاكتساب نتيجة
       جانبية بتتقاس برضه في offerImpact. */
    goal: "basket",
    from: "2026-09-17",
    until: "2026-09-30",
    ringsAs: "gift_line",
    /* مفيش صف كتالوج: ده شرط على الفاتورة مش منتج يتباع. صف في feed.csv
       بسعر ٩٦ معناه إن ميتا تعرض «منتج» اسمه هدية بسعر ٩٦ — وده كذب. */
    catalogRow: false,
    detect: { mode: "threshold", min: 96 },
    components: [
      { label: "فاتورة ٩٦ ريال أو أكثر" },
      { label: "بيتزا تشيكن رانش هدية" },
    ],
  },
];
    ...
    return {
      ...
      flags: [
        /* الملاحظة دي كانت بتطلع على أي عرض فيه طلبات Take away — حتى لو
           ...
           (زي nd96) كان هيتبلّغ عن سلوك سليم تمامًا، والحارس اللي بينبّه على
