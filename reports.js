  ...
  async function buildReport(period, date) {
    ...
    const ads = {
      ...
      platforms: scoreRows.map((r) => ({
        ...
        /* «النتيجة» بالتعريف الواحد (ads.js/META_RESULT_MODEL): نية طلب
           (دوسة واتساب / محادثة) + أي شرا أونلاين. كانت بتتحسب في الـ
           scorecard وتتقفل هنا من غير ما توصل التقرير — فالتقرير كان لسه
           بيتكلم عن الدنيا القديمة. null = عمى قياس مش صفر. */
        platformResults: r.platformResults ?? null,
        platformResultBasis: r.platformResultBasis ?? null,
        platformCostPerResult: r.platformCostPerResult ?? null,
    ...
    const attrib = att ? {
      ...
      /* الجملة دي كانت بتقول «تحويلات المنصة بتقرا صفر رغم الصرف — ده متوقّع
         مش عطل». كانت صح لما القارئ كان بيعدّ purchase وبس، وبقت **غلط
         وخطيرة** بعد التصليح: دلوقتي الرقم بيقرا نية الطلب وبيطلع أكبر من
         صفر. لو سبناها كنا بنعلّم المالك إنه يتجاهل صفر حقيقي — والصفر
         الحقيقي دلوقتي هو الإشارة اللي قاعدة القتل بتشتغل عليها. */
      instoreTruth: "الشراء اللي بيتقفل على الكاشير بيتبعت للمنصات بـ action_source = physical_store. ده الوصف الصح للبيع، ونتيجته إن المنصة بتقبل الحدث بس **مبتنسبهوش لحملة بعينها** زي ما بتعمل مع شراء على الموقع — يعني عمود «الشرا» عند المنصة هيفضل صفر للأبد، وده متوقّع مش عطل. عشان كده «نتيجة» عندنا مش معناها شرا: معناها **نية طلب** — دوسة «اطلب على واتساب» (حدث Contact/Lead) أو محادثة واتساب ابتدت من الإعلان. الرقم ده بيقرا أكبر من صفر فعلاً، ولو قرا صفر مع صرف حقيقي يبقى دي إشارة تستاهل التوقّف عندها مش عمى قياس. والعمود اللي جنبه (التحويلات المؤكدة) فاضل دليلنا إحنا على الفلوس اللي دخلت الدرج.",
    } : null;
