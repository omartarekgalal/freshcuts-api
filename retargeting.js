...
export function register(app, ctx, deps = {}) {
  ...
  async function pixelLayer() {
    ...
    return {
      ...
      /* أكبر بذرة مؤكّدة = أكبر صف حجمه متقاس فعلاً (الحدّين مختلفين).
         الرقم ده بيتقرا حيّ من aud_sizes كل مرة — كان مكتوب بالإيد
         (٦٣٠٠–٧٤٠٠) فكان بيفضل زي ما هو مهما الجمهور كبر أو صغر. */
      confirmedLargeSeed: (() => {
        const best = rows
          .filter((r) => r.measured && !r.isSentinel && r.monthlyReach)
          .sort((a, z) => z.monthlyReach.high - a.monthlyReach.high)[0];
        if (!best) {
          return {
            audience: null, monthlyReach: null, boundsDiffer: false, measuredAt: null,
            why: "لسه مفيش ولا جمهور رجّع حدّين مختلفين — يعني مفيش بذرة حجمها مؤكّد. مش بنكتب رقم قديم مكان القياس.",
            soWhat: "الشبيه اللي فشل بـ٤٣٫٦٦ ر.س كان مبني على بذرة صغيرة كانت بتقرا 1000/1000. قبل ما نجرّب تاني لازم نشوف قياس حقيقي هنا.",
          };
        }
        return {
          audience: best.ar, key: best.key,
          monthlyReach: { low: best.monthlyReach.low, high: best.monthlyReach.high },
          boundsDiffer: true, measuredAt: best.measuredAt,
          why: "الحدّين مختلفين، يعني ده قياس مش أرضية — وده أكبر بذرة حجمها مؤكّد عندنا دلوقتي.",
          soWhat: "الشبيه اللي فشل بـ٤٣٫٦٦ ر.س كان مبني على بذرة صغيرة كانت بتقرا 1000/1000 فبانت كبيرة وهي مش كده. البذرة دي مختلفة تمامًا وتستاهل تتجرّب من أول وجديد بعد الشير.",
        };
      })(),
      note: anyMeasured
