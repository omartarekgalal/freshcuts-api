...
export function audienceSizeVerdict(a = {}) {
  ...
  if (sentinel) {
    return { measured: false, sentinel: true, size: null, floor: lo,
      ...
      why: `"${name}" — ميتا بتقول ${lo} في الحد الأدنى **والأعلى مع بعض**، وده رقم أرضية مش عدد. المعنى الوحيد المضمون: «عند الأرضية أو تحتها». بذرة «عملاء VIP» طلعت بنفس الشكل (١٠٠٠/١٠٠٠) وهي وقتها (أغسطس ٢٠٢٦) كانت ٧٧ شخص بس — والعدد الحقيقي بيتحسب حيّ من شرائحنا (audiences.js)، مش من رقم مكتوب. ${dc === 200 ? "ينفع يتحط في استهداف/استبعاد، بس" : ""} **ماينفعش يبقى بذرة LAL** — العدد الحقيقي لازم ييجي من داتانا إحنا (شرائح audiences.js)، مش من الرقم ده.` };
  }
