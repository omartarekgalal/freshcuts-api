...
/* ⭐ تقييم جوجل الحي — reviews.js بيحدّثه من settings.googlePlace (كل ٦ ساعات من
   Places API). مفيش رقم مكتوب هنا: الرقم بيتغيّر (٤٫٨ ← ٤٫٩) والنص بيتقري وقت الطلب. */
let liveGoogle = null;
export function setLiveGoogleRating(p) {
  const r = Number(p && p.rating), n = Math.floor(Number(p && p.count));
  if (r > 0 && r <= 5 && n >= 1) liveGoogle = { rating: r, count: n };
}
const arD = (v) => String(v).replace(/\d/g, (d) => "٠١٢٣٤٥٦٧٨٩"[d]).replace(".", "٫");
/* «⭐٤٫٩ على جوجل (+١٨٠ تقييم)» — العدد مقرّب لتحت لأقرب ١٠ */
export function googleRatingText() {
  if (!liveGoogle) return "تقييمنا على جوجل (الرقم الحي في اللوحة)";
  const n = liveGoogle.count, c = n < 10 ? arD(n) : `+${arD(Math.floor(n / 10) * 10)}`;
  return `⭐${arD(liveGoogle.rating.toFixed(1))} على جوجل (${c} تقييم)`;
}
...
export const PLAYBOOK = {
  ...
  creative: {
    ...
    get winners() { return `KILO-A (شبكة ٤ مشاوي «اختار كيلوك»: CTR ٦٫٤٪) و BOX-B (٦ أصناف بأساميهم: أعلى ATC). الدليل ${googleRatingText()} بيتحط في النص — الرقم الحي وقت كتابة الإعلان، مش رقم محفوظ.`; },
  ...
  platformRoles: [
    ...
    { platform: "google", get role() { return `نية بحث/خرائط (${googleRatingText()}) — الإعلانات مش شغالة؛ البحث المجاني بيجيب أعلى معدل تحويل.`; } },
